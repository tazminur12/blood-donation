...
export default function PoliceServicePage() {
  ...
  const filteredStations = useMemo(() => {
    return policeStations.filter((station) => {
      ...
      const matchesThana = !selectedThana || station.name === selectedThana;

      const matchesService =
        !selectedService || station.services?.includes(selectedService);

      return matchesSearch && matchesThana && matchesService;
    });
  }, [policeStations, searchTerm, selectedThana, selectedService]);
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex flex-wrap justify-center gap-2">
                      {(station.services || []).map((service, index) => (
                        <div
                          key={index}
                          className="bg-white border border-gray-200 text-gray-700 px-3 py-2 rounded-full text-xs font-semibold hover:border-blue-300 hover:text-blue-700 hover:bg-blue-50 transition-all duration-300 flex items-center gap-1 shadow-sm"
                        >
        ...
        {!loading && filteredStations.length > 0 && (
          <div className="mt-8 text-center p-6 bg-white rounded-lg shadow-lg border border-gray-100">
            ...
            <p className="text-gray-600 text-sm mb-3">
              সর্বদা আপনার নিরাপত্তার জন্য প্রস্তুত
            </p>
