...
export default function CampaignsPage() {
  ...
  useEffect(() => {
    ...
    // Only load campaigns if user is admin
    if (session?.user?.role === "admin") {
      loadCampaigns();
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-1">
            সমস্ত রক্তদান ক্যাম্পেইনের তথ্য দেখুন এবং পরিচালনা করুন
          </p>
        </div>
      </div>
                      ...
                      <div className="space-y-1">
                        {campaign.location ? (
                          ...
                          <>
                            ...
                            {campaign.district && (
                              <p className="text-xs text-slate-500 ml-5">{campaign.district}</p>
                            )}
                            {campaign.upazila && (
                              <p className="text-xs text-slate-500 ml-5">{campaign.upazila}</p>
                            )}
                          </>
                        )}
                        {!campaign.location && !campaign.division && (
