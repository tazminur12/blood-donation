...
export default function NewsPage() {
  ...
  const handleEdit = (newsItem) => {
    ...
    setFormData({
      ...
      newspaperName: newsItem.newspaperName || "",
  ...
  const handleDelete = async (newsId, newsTitle) => {
    ...
    try {
      const res = await fetch(`/api/admin/news?newsId=${newsId}`, {
        method: "DELETE",
      });
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-blue-200 p-4 shadow-sm">
          ...
          <p className="text-2xl font-bold text-blue-700">
            {new Set(news.map((n) => n.newspaperName).filter(Boolean)).size}
          </p>
        </div>
      </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="উদাহরণ: রক্তদান, ইভেন্ট, ক্যাম্পেইন"
                    className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  />
                </div>
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-2">
                    <FaNewspaper className="inline mr-2 text-blue-600" />
                    পত্রিকার নাম
                  </label>
                  <input
                    type="text"
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, newspaperName: e.target.value })
                    }
                    placeholder="উদাহরণ: দৈনিক প্রথম আলো"
                    className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  />
                </div>
              </div>
